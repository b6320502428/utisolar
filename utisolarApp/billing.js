...
export default function Billing() {
    ...
    const calNormel2 = () => {
        ...
        var vft = sumMonth * ft
        var vprn
        if (maxOnPeak1 > (maxOnPeak2 * 0.61974))
            vprn = (maxOnPeak1 - (maxOnPeak2 * 0.61974)) * billingType.kvarcharge
        else
            vprn = 0
            ...
            <ScrollView style={styles.container}>
                <View style={styles.dataContainer}>
                    ...
                    <TouchableOpacity style={styles.button} onPress={handleSelectDate}>
                        <Text style={styles.buttonText}>Select Month ({date})</Text>
                    </TouchableOpacity>
                    {!isHidden && (
                        <MonthYearExample />
                    )}
                    ...
                    {billingType !== null && (billingType.peatype === "1112" || billingType.peatype === "1122" || billingType.peatype === "2112" || billingType.peatype === "2122") && (
                        <View>
                            <Text style={styles.buttonText}>Bill of ({billingDate})</Text>
                            ...
                            <TouchableOpacity style={styles.row}>
                                ...
                                {maxOnPeak1 > (maxOnPeak2 * 0.61974) ? (
                                    <Text style={styles.rowText}>{(maxOnPeak1 - (maxOnPeak2 * 0.61974)).toFixed(2)}</Text>
                                ) : (
                                    <Text style={styles.rowText}>0</Text>
                                )}
                                <Text style={styles.rowText}>กิโลวาร์</Text>
                            ...
                            <TouchableOpacity style={styles.row}>
                                ...
                                {maxOnPeak1 > (maxOnPeak2 * 0.61974) ? (
                                    <Text style={styles.rowText}>{((maxOnPeak1 - (maxOnPeak2 * 0.61974)) * billingType.kvarcharge).toFixed(2)}</Text>
                                ) : (
                                    <Text style={styles.rowText}>0</Text>
                                )}
                                <Text style={styles.rowText}>บาท</Text>
